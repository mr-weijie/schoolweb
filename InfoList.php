...
<tr><TD>
<?php
$funid=trim($_GET['funid']);
$rowid= trim($_GET['rowid']);
$Flag= trim($_GET['flag']);
$CurPage= trim($_GET['CurPage']);
if($CurPage=='') $CurPage=1;
	  $SqlStr="SELECT FunId,FunctionName,image from webfunction where RowID='".$funid."'";
	//echo $SqlStr;
	  $rs = mysql_query($SqlStr);
		...
		$Root=$row['FunId'];	
		$RootFN=$row['FunctionName'];	
		$image=$row['image'];	
		mysql_free_result($rs);

?>
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" height="74"><tbody>
    <tr><td background="<?php echo $image?>"></td></tr></tbody></table>
<!------------------页面开始了----------------------->
	...
	<tr>
		<td height="100%">
        	<table width="156" border="0" cellspacing="0" cellpadding="0" height="100%" background="./images/ManageFunListBackGround.gif" class="ChildFunction">
            	<tbody>
<?php
			$SqlStr="SELECT RowID,FunctionName,ManageFunId  from webfunction where Root='".$Root."' And Deep = 3 and IsManage=1 and ManageFunId >0 AND IsUse = 1 order by OrderID";
			//echo $SqlStr;
		  $rs = mysql_query($SqlStr);
			if($rs=='')
			{
...
//			$FunctionDir=$row['FunctionDir'];
			...
			echo '<td><a href="?funid='.$funid.'&rowid='.$RowID.'&flag='.$ManageFunId.'&FN='.urlencode($FunctionName).'">&nbsp;&nbsp;'.$FunctionName.'</a></td>';
                    ...
                    <tr>
                        	...
                        	<table class="indicator" cellpadding="0" cellspacing="0" width="100%" height="50" border="0">
                            	<tbody>
                                	<tr>
                                        ...
                                        <td background="images/indicator_center.gif">当前位置 &gt;&gt; <?php echo $RootFN?> &gt;&gt; <?php echo $FunctionName?></td>
                     ...
                     <tr>
                            ...
                            <table border="0" cellpadding="6" cellspacing="1" width="98%">
                               <tbody>
                               <?php listNewsInfo($rowid,$Flag,15,$CurPage); ?>
                               </tbody>
                            </table>
...
<?php 
function listNewsInfo($RowID,$Flag,$Nums,$CurPage)
{
	$SqlStr="SELECT FunId,FunctionLink from webfunction where RowID='".$RowID."'";
	//echo $SqlStr;
	$rs = mysql_query($SqlStr);
	$row = mysql_fetch_array($rs);
	$FunId=$row['FunId'];
	$FunctionLink=$row['FunctionLink'];
	mysql_free_result($rs);
	if($FunId==0&&strlen($FunctionLink)>0)
	{
		echo '<iframe src="./'.$FunctionLink.'?flag='.$Flag.'" border="0" width="100%" height="100%" frameborder="0"></iframe>';
		return;//查询类模块单独调用程序
	}else 
	{
			if($FunId>0&&strlen($FunctionLink)>0)
	...
	$SqlStr="SELECT count(*) As Recs from tblcontent where Flag=".$FunId;
	//echo $SqlStr;
	$rs = mysql_query($SqlStr);
	$row = mysql_fetch_array($rs);
	$Recs=$row['Recs'];
	...
	$SqlStr="SELECT RowID,Title from tblcontent where Flag=".$FunId." order by OutTime Desc limit ".($CurPage-1)*$Nums.",".$Nums;
	$rs = mysql_query($SqlStr);
	$I=0;
 	while($row = mysql_fetch_array($rs)) 
	{

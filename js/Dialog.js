var IMGFOLDERPATH = '/DialogImg/';//图片路径配置
...
if ( !this.ParentWindow ){
	...
	arr.push("                <div id='_Message_"+this.ID+"'>&nbsp;</div><br/></td>");
...
//	arr.push("           	<input id='_ButtonOK_"+this.ID+"'  type='button' onclick=\"Dialog.getInstance('"+this.ID+"').close();\" value='确 定'>");
	...
	if(this.ShowMessageRow){
		...
		if(this.Message){
			pw.$("_Message_"+this.ID).innerHTML = "<div align=center><font style=\"color:sienna;FONT-SIZE:18pt;\">"+this.Message+"</font></div>";
		}
	}

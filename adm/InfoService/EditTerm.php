...
if(strlen($Act)>0)//说明是保存操作
{
	...
	if(strlen($rowid)==0)
	{
		$rowid=strtoupper(md5($TermName.$Flagrowid.$Flag.date("Y-m-d H:i:s")));//最后转换成大写
		$SqlStr="Insert Into InfoSearch_term(RowID,Flag,TermName) Values('".$rowid."','".$Flag."','".$TermName."')";
	}else
	{
		$SqlStr="Update InfoSearch_term Set TermName='".$TermName."' Where RowID='".$rowid."'";
	}
	$rs=mysql_query($SqlStr);
	if($rs)//操作成功
	{
		$retstr="<script>alert('记录提交成功!');window.location.href='TermList.php?rowid=".$Flagrowid."&CurPage=".$CurPage."'; </script>";
	}else//操作失败
	{
...
if(strlen($rowid)>0)
{
	$SqlStr="Select Id,Flag,TermName from InfoSearch_term Where RowID='".$rowid."'";
	$rs = mysql_query($SqlStr);
	if($rs=='')
	{
...
<form name="FrmManage" action="EditTerm.php"  method="post">
...
<tr><td align="right"><font color="red"> *</font>学期名称</td><td align="left"><input type="text" name="TermName"  value="<?php echo $TermName?>" size="50" maxlength="200"></td></tr>
...
<input type="hidden" name="NotNullDesc" value="标识字段,学期名称" >

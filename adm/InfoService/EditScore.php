...
if($Act==1)
{
	...
	$Title=$_POST['Title'];
	$ScoresContent=$_POST['content'];
	if(strlen($rowid)==0)
	{
	//ID,RowID,Flag,ClassName,TermName,TestType,Title,ScoresContent
		$rowid=strtoupper(md5($Title.$ScoresContent.$Flag.date("Y-m-d H:i:s")));//最后转换成大写
		$SqlStr="Insert Into InfoSearch_scores(RowID,Flag,ClassName,TermName,TestType,Title,ScoresContent) Values('".$rowid."','".$Flag."','".$ClassName."','".$TermName."','".$TestType."','".$Title."','".$ScoresContent."')";
	}else
	{
		$SqlStr="Update InfoSearch_scores Set ClassName='".$ClassName."',TermName='".$TermName."',TestType='".$TestType."',Title='".$Title."',ScoresContent='".$ScoresContent."' Where RowID='".$rowid."'";
	}
	$rs=mysql_query($SqlStr);
	if($rs)//操作成功
	{
		$retstr="<script>alert('记录提交成功!');window.location.href='ScoreList.php?rowid=".$FlagRowID."&CurPage=".$CurPage."'</script>";
	}else//操作失败
	{
...
if(strlen($rowid)==32)
{
	$SqlStr="Select ID,RowID,Flag,ClassName,TermName,TestType,Title,ScoresContent from InfoSearch_scores Where RowID='".$rowid."'";
	$rs = mysql_query($SqlStr);
...
<tr><td height="25" align="right"><font color="red"> *</font>班级名称&nbsp;</td><td height="25" align="left">
<select name="ClassName">
<option value="0">选择班级名称</option>
<?php 
	$SqlStr="Select ClassName from InfoSearch_class order by ClassName";
	$rs = mysql_query($SqlStr);
  	while($row = mysql_fetch_array($rs)) 
	{
		$Name=$row['ClassName'];
    	echo '<option value="'.$Name.'" ';
		if($Name==$ClassName) echo 'selected';
		echo '>'.$Name.'</option>';
	}
	mysql_free_result($rs);
?>
</select>&nbsp;&nbsp;
<input type="button" id="btnEditContent" value="编辑文章内容" onclick="javascript:editContent();"  onmouseover="javascript:changebtn(this);" onmouseout="javascript:changebtn(this);"></td></tr>
...
<tr><td height="25" align="right"><font color="red"> *</font>考试类型&nbsp;</td><td height="25" align="left">
<select name="TestType">
<option value="0">选择考试类型</option>
<?php 
	$SqlStr="Select Type from microtype Where FieldName='TestType' order by ID";
	$rs = mysql_query($SqlStr);
  	while($row = mysql_fetch_array($rs)) 
	{
		$Type=$row['Type'];
    	echo '<option value="'.$Type.'" ';
		if($Type==$TestType) echo 'selected';
		echo '>'.$Type.'</option>';
	}
	mysql_free_result($rs);
?>
...
<tr><td height="25" align="right"><font color="red"> *</font>成绩表名称&nbsp;</td><td height="25" align="left"><input type="text" name="Title" value="<?php echo $Title?>" size="50" maxlength="200">&nbsp;&nbsp;</td></tr>
<tr><td align="right" valign="top"><font color="red"> *</font>成绩内容&nbsp;</td>
...
<tr><td width="75"></td><td width="559">
            ...
            <input type="hidden" name="ArticleRowID" value="<?php echo $ArticleRowID?>">
 			<input type="hidden" name="NotNullFeild" value="Flag,ClassName,TermName,TestType,Title,Content">
            <input type="hidden" name="NotNullDesc" value="标识字段,班级名称,学期名称,考试类型,成绩表名称,成绩内容" > 			

...
if($Act!='') 
{
	DeleteRecord("infosearch_teakebiao",$_POST['DelRowID']);//删除目标记录
}
...
if(strlen($rowid)==0) $rowid=trim($_POST['Flagrowid']);
            ...
            <tr> 
			  <td height="100%" align="center" valign="top">
				...
				<td class="right"></td>
				  ...
				  <tr valign="top">
		      <td height="100%" valign="top"><?php TeacherClassList()?></td></tr></tbody></table></td></tr></tbody></table>
</body></html>



<?php function TeacherClassList()
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="ManageList">
  ...
  <tbody><tr>
    ...
    <tbody>
    <tr><th width="40">序号</th><th>学期名称</th><th>教师名称</th><th nowrap="" width="40">选择</th></tr>
    <?php
	$Nums=15;//每页最多显示信息条数
	$ConStr="Where Flag='".$Flag."'";
	$SqlStr="SELECT count(*) As Recs from infosearch_teakebiao ".$ConStr;
	$rs = mysql_query($SqlStr);
	$row = mysql_fetch_array($rs);
	$Recs=$row['Recs'];
	$pages=round($Recs/$Nums+0.499);//总页数
	if($CurPage>$pages) $CurPage=$pages; 
	$SqlStr="Select RowID,TeacherName,TermName,Title from infosearch_teakebiao ".$ConStr." Order by ID limit ".($CurPage-1)*$Nums.",".$Nums;;
	$rs = mysql_query($SqlStr);
	$XH=0;
  	while($row = mysql_fetch_array($rs)) 
	{
    	...
    	echo '<tr><td align="center">'.$XH.'</td><td><A href="EditTeacherClass.php?rowid='.$RowID.'&flagrowid='.$rowid.'&CurPage='.$CurPage.'">'.$TermName.'&nbsp;</a></td><td width="200">'.$TeacherName.'</td><td align="center"><input type="checkbox" name="Id" value="'.$RowID.'"></td></tr>';
	}
	mysql_free_result($rs);

	?>
	</tbody></table></td></tr>
    <tr><td colspan="3" height="40"><div align="right"><input name="add" type="button" style="BACKGROUND-IMAGE: url(/adm/images/add.gif)" class="ManageButton" alt="增加" onClick="javascript:AddRecord('<?php echo "EditTeacherClass.php?flag=".$Flag."&flagrowid=".$rowid ?>');" onMouseOver="javascript:changebtn(this);" onMouseOut="javascript:changebtn(this);">&nbsp;&nbsp;<input name="del" type="button" style="BACKGROUND-IMAGE: url(/adm/images/del.gif)" class="ManageButton" alt="删除" onClick="javascript:DelRecord(1);" onMouseOver="javascript:changebtn(this);" onMouseOut="javascript:changebtn(this);"><input name="Act" type="hidden"><input name="DelRowID" type="hidden">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div></td></tr>
<?php
	if($pages>1)
	{

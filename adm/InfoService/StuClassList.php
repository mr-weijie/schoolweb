...
if($Act!='') 
{
	DeleteRecord("infosearch_stukebiao",$_POST['DelRowID']);//删除目标记录
}
            ...
            <tr> 
			  <td height="100%" align="center" valign="top">
				...
				<td class="right"></td>
				  ...
				  <tr valign="top">
		      <td height="100%" valign="top"><?php StuClassList()?></td></tr></tbody></table></td></tr></tbody></table>
</body></html>



<?php function StuClassList()
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="ManageList">
  ...
  <tbody><tr>
    ...
    <tbody>
    <tr><th width="40">序号</th><th>学期名称</th><th>班级名称</th><th nowrap="" width="40">选择</th></tr>
    <?php
	$Nums=15;//每页最多显示信息条数
	$ConStr="Where Flag='".$Flag."'";
	$SqlStr="SELECT count(*) As Recs from infosearch_stukebiao ".$ConStr;
	$rs = mysql_query($SqlStr);
	$row = mysql_fetch_array($rs);
	$Recs=$row['Recs'];
	$pages=round($Recs/$Nums+0.499);//总页数
	if($CurPage>$pages) $CurPage=$pages; 
	$SqlStr="Select ID,RowID,Flag,ClassName,TermName,KeBiaoContent from infosearch_stukebiao ".$ConStr." Order by ID limit ".($CurPage-1)*$Nums.",".$Nums;;
	$rs = mysql_query($SqlStr);
	$XH=0;

...
if($Act!='') 
{
	DeleteRecord("Qikan",$_POST['DelRowID']);//删除目标记录
}
            ...
            <tr> 
			  <td height="100%" align="center" valign="top">
				...
				<td class="right"></td>
				  ...
				  <tr valign="top">
		      <td height="100%" valign="top"><?php QikanList()?></td></tr></tbody></table></td></tr></tbody></table>
</body></html>



<?php function QikanList()
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="ManageList">
  ...
  <tbody><tr>
    ...
    <tbody>
    <tr><th width="40">序号</th><th>期刊编号</th><th>期刊名称</th><th>刊号</th><th>出版社</th><th nowrap="" width="40">选择</th></tr>
    <?php
	$Nums=15;//每页最多显示信息条数
	$ConStr="Where Flag='".$Flag."'";
	$SqlStr="SELECT count(*) As Recs from Qikan ".$ConStr;
	$rs = mysql_query($SqlStr);
	$row = mysql_fetch_array($rs);
	$Recs=$row['Recs'];
	$pages=round($Recs/$Nums+0.499);//总页数
	if($CurPage>$pages) $CurPage=$pages; 
	$SqlStr="Select RowID,Flag,ISDN,Name,Kanhao,Publish from Qikan ".$ConStr." Order by ID limit ".($CurPage-1)*$Nums.",".$Nums;;
	$rs = mysql_query($SqlStr);
	$XH=0;
  	while($row = mysql_fetch_array($rs)) 
	{
		...
		$Kanhao=$row['Kanhao'];
		$Publish=$row['Publish'];
    	echo '<tr><td align="center">'.$XH.'</td><td><A href="EditQikan.php?rowid='.$RowID.'&CurPage='.$CurPage.'">'.$ISDN.'&nbsp;</a></td><td><A href="EditQikan.php?rowid='.$RowID.'&CurPage='.$CurPage.'">'.$Name.'&nbsp;</a></td><td><A href="EditQikan.php?rowid='.$RowID.'&CurPage='.$CurPage.'">'.$Kanhao.'&nbsp;</a></td><td><A href="EditQikan.php?rowid='.$RowID.'&CurPage='.$CurPage.'">'.$Publish.'&nbsp;</a></td><td align="center"><input type="checkbox" name="Id" value="'.$RowID.'"></td></tr>';
	}
	mysql_free_result($rs);

	?>
	</tbody></table></td></tr>
    <tr><td colspan="3" height="40"><div align="right"><input name="add" type="button" style="BACKGROUND-IMAGE: url(/adm/images/add.gif)" class="ManageButton" alt="增加" onClick="javascript:AddRecord('<?php echo "EditQikan.php?flag=".$Flag?>');" onMouseOver="javascript:changebtn(this);" onMouseOut="javascript:changebtn(this);">&nbsp;&nbsp;<input name="del" type="button" style="BACKGROUND-IMAGE: url(/adm/images/del.gif)" class="ManageButton" alt="删除" onClick="javascript:DelRecord(1);" onMouseOver="javascript:changebtn(this);" onMouseOut="javascript:changebtn(this);"><input name="Act" type="hidden"><input name="DelRowID" type="hidden">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div></td></tr>
<?php
	if($pages>1)
	{

...
if($Act==1)
{
	...
	if(strlen($rowid)==0)
	{
		...
		$SqlStr="Insert Into Tushupingjia(RowID,Flag,Name,Author,Content) Values('".$rowid."','".$Flag."','".$Name."','".$Author."','".$Content."')";
	}else
	{
		$SqlStr="Update Tushupingjia Set Name='".$Name."',Author='".$Author."',Content='".$Content."' Where RowID='".$rowid."'";
	}
	$rs=mysql_query($SqlStr);
	if($rs)//操作成功
	{
		$retstr="<script>alert('记录提交成功!');window.location.href='PingjiaList.php?rowid=".$FlagRowID."&CurPage=".$CurPage."'</script>";
	}else//操作失败
	{
...
if(strlen($rowid)==32)
{
	$SqlStr="Select ID,RowID,Flag,Name,Author,Content from Tushupingjia Where RowID='".$rowid."'";
	$rs = mysql_query($SqlStr);
...
<?php if(strlen($rowid)>0) echo '<input type="button" id="btnEditContent" value="编辑评介内容" onclick="javascript:editContent();"  onmouseover="javascript:changebtn(this);" onmouseout="javascript:changebtn(this);">' ?></td></tr>
...
<tr><td align="right" valign="top"><font color="red"> *</font>评介内容&nbsp;</td>
...
<tr><td width="75"></td><td width="559">
	        ...
	        <input type="hidden" name="ArticleRowID" value="<?php echo $ArticleRowID?>">
 			<input type="hidden" name="NotNullFeild" value="Flag,Name,Author,content">
            <input type="hidden" name="NotNullDesc" value="标识字段,图书名称,图书作者,评介内容" > 			
            <input type="hidden" name="EregPattern" value="d,C,C,C"></td></tr>

...
if(strlen($Act)>0)//说明是保存操作
{
	...
	if(strlen($rowid)==0)
	{
		$rowid=strtoupper(md5($LinkName.$Link.$Flag.date("Y-m-d H:i:s")));//最后转换成大写
		$SqlStr="Insert Into friendlinks(RowID,Flag,LinkName,Link) Values('".$rowid."','".$Flag."','".$LinkName."','".$Link."')";
	}else
	{
		$SqlStr="Update friendlinks Set LinkName='".$LinkName."',Link='".$Link."' Where RowID='".$rowid."'";
	}
	$rs=mysql_query($SqlStr);
	if($rs)//操作成功
	{
		$retstr="<script>alert('记录提交成功!');window.location.href='FrdLnkSet.php?rowid=".$Flagrowid."'; </script>";
	}else//操作失败
	{
...
if(strlen($rowid)>0)
{
	$SqlStr="Select friendlinks.LinkName,friendlinks.Link,managefunction.FunctionName,managefunction.RowID,managefunction.Flag From friendlinks inner join managefunction on friendlinks.flag=managefunction.flag  Where friendlinks.RowID='".$rowid."'";
	$rs = mysql_query($SqlStr);
	if($rs=='')
	{
...
<form name="FrmManage" action="EditFrdLnk.php"  method="post">
...
<tr><td align="right"><font color="red"> *</font>连接名称</td><td align="left"><input type="text" name="LinkName"  value="<?php echo $LinkName?>" size="50" maxlength="200"></td></tr>
<tr><td align="right"><font color="red"> *</font>连接地址</td><td align="left"><input type="text" name="Link" value="<?php echo $Link?>" size="80" maxlength="512"></td></tr>
<tr><td width="18%"></td><td width="82%">
<input type="hidden" name="RowId" value="<?php echo $rowid?>" />
<input type="hidden" name="Flagrowid" value="<?php echo $Flagrowid?>" />
<input type="hidden" name="NotNullFeild" value="Flag,LinkName,Link">
<input type="hidden" name="NotNullDesc" value="标识字段,连接名称,连接地址" >
